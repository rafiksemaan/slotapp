...
// Ensure this script is only accessed via POST request with JSON content
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || strpos($_SERVER['CONTENT_TYPE'], 'application/json') === false) {
    // If not a JSON POST request, redirect or return an error
    // For now, we'll just exit if it's not the expected request type
    // In a production environment, you might want to return a JSON error response
    // or redirect to the main meters page if accessed directly.
...
function processMeterData($meter_data, $operation_date, $original_filename, $conn) {
    try {
        ...
        foreach ($meter_data as $i => $row_data) {
            ...
            $machine_number_from_file = trim($row_data_lower['machine'] ?? ''); // Assuming 'machine' column in XLSX
